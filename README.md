...
You can test against specific database versions:

```bash
# PostgreSQL 14
POSTGRES_VERSION=14 docker compose up -d postgres

# MySQL 8.0
MYSQL_VERSION=8.0 docker compose up -d mysql
```
...
Generate HTML and Clover coverage reports:

```bash
XDEBUG_MODE=coverage ./vendor/bin/paratest --testsuite Unit
```

Reports are generated in `coverage/html/` and `coverage/clover.xml`.
